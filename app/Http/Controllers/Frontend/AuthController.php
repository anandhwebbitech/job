...
class AuthController extends Controller
{
    ...
    public function employeeSingleLogin(Request $request)
    {
        $type = $request->query('type', 'jobseeker');
        ...
        return view('frontend.auth.single-login', compact('skills','qualifications','type'));
    }
    ...
    public function employerRegisterSubmit(Request $request)
    {
        try {
            ...
            $validator = Validator::make($request->all(), [
                ...
                'branch_mobile.*'  => 'nullable|digits:10',
            ...
            $employer = EmployerDetail::create([
            ...
            if ($request->branch_name) {
                ...
                foreach ($request->branch_name as $key => $branchName) {
                    ...
                    if (
                        empty($branchName) &&
                        empty($request->branch_email[$key]) &&
                        empty($request->branch_mobile[$key]) &&
                        empty($request->branch_address[$key])
                    ) {
                        continue;
                    }

                    EmployerBranch::create([

                        'employer_id'    => $employer->id,
                        'branch_name'    => $branchName,
                        'branch_email'   => $request->branch_email[$key] ?? null,
                        'branch_mobile'  => $request->branch_mobile[$key] ?? null,
                        'branch_address' => $request->branch_address[$key] ?? null,
                    ]);
                }
            }
            ...
            if ($admin) {
                ...
                $message = Notification::typeName(
                    Notification::TYPE_NEW_COMPANY_REGISTER
                );
